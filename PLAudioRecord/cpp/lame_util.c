...
Java_com_pengl_mp3lib_LameUtil_init(
        JNIEnv
        *env,
        jclass cls, jint
        inSamplerate,
...
JNICALL Java_com_pengl_mp3lib_LameUtil_encode(
...
JNICALL Java_com_pengl_mp3lib_LameUtil_flush(
...
JNIEXPORT void JNICALL
Java_com_pengl_mp3lib_LameUtil_close
        (JNIEnv
         *env,
         jclass cls
        ) {
 ...
 * wav转换mp3
 */
JNIEXPORT void JNICALL Java_com_pengl_mp3lib_LameUtil_convert
        (JNIEnv *env, jclass cls, jstring jwav, jstring jmp3, jint inSampleRate) {
    ...
    do {
        ...
        read = (int) fread(wav_buffer, sizeof(short int) * channel, 8192, fwav);
        ...
        //把转化后的mp3数据写到文件里
        fwrite(mp3_buffer, 1, (size_t) write, fmp3);
